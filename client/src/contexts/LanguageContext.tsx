...
const translations = {
  en: {
    ...
    "howItWorks.step2Desc": "When posting, students submit a problem and their proposed solution at the same time. Posts are published immediately after passing an automatic content filter. From the detail page, other users can also add further solution proposals. Example solution: 'Introduce a pre-order system (morning deadline) for popular items instead of first-come, first-served.'",
  ...
  ja: {
    ...
    "howItWorks.step2Desc": "意見を投稿する際には、問題文と合わせて自分の解決策も一緒に提出します。投稿は自動フィルターを通過すると即時公開されます。また詳細ページから、他のユーザーが追加の解決策を提案することもできます。例：「人気メニューについて、数量限定ではなく事前予約制（当日午前締切）を導入する。」",
    "howItWorks.step3Title": "03 / VOTE",
