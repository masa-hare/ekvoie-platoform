...
type DeletionReason = "personal_information" | "harassment_or_hate" | "threat_or_illegal_content" | "off_topic_or_spam" | "other_policy_violation";
...
export default function Admin() {
  ...
  const [deletionReason, setDeletionReason] = useState<DeletionReason>("personal_information");
  ...
  const opinionsByTheme = useMemo(() => {
    ...
    for (const theme of themes ?? []) grouped.set(theme.id, (opinions ?? []).filter(opinion => opinion.themeId === theme.id));
    return grouped;
  }, [opinions, themes]);
